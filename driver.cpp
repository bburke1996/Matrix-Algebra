...
void menu() {
	...
	while (1) {
		/* Initialize all global variable to 0. This is especially useful when a user returns to the main menu muliple times. */
		...
		while (1) {                                                 // Get operation input from the user
			cout << "Choose an operation:" << endl << "1) Scalar Multiplication" << endl << "2) Matrix Addition" << endl <<  "3) Matrix Multiplication" << endl << "4) Exit" << endl << endl;
			cin >> input;
...
void matrixMultiplication() {
	...
	cout << endl << "Matrix Multiplication in Matrix Algebra is when one matrix is multiplied by another matrix, e.g. A x B = C where A, B, C are matricies. The number of columns of the first matrix must equal the number of rows of the second matrix, otherwise their product is undefined. So, for the given example, the number of columns of A must equal the number of rows of B. If A is and m x n matrix and B is an n x p matrix, the C will be an m x p matrix. The (i, j)th entry of matrix C is equal to the dot product of the ith row of A and the jth column of B." << endl << endl;
	...
	/* Check that the number of columns of the first matrix is equal to the number of rows of the second matrix. If not, return to the main menu. */
	if (columns1 != rows2) {
		cout << "The number of columns of the first matrix must be equal to the number of rows of the second matrix. " << endl << endl;
		menu();
	} 

	Matrix m2 = Matrix(rows2, columns2);
	/* These nested for loops ask the user for each entry's value and updates the values in the second matrix. */
	for (i = 1; i <= rows2; i++) {
	...
	cout << "The product of the two matricies you entered is: " << endl << endl;
	m3.printMatrix();
}
...
void scalarMultiply() {                                          
	...
	/* Getting the scalar quantity from the user that the matrx will be multiplied by */
	...
	cout << endl << "Your matrix after being multiplied by " << scalarNum << ": " << endl << endl;
	m1.printMatrix();
}
...
void matrixAddition() {
	...
	/* Printing out the first matrix to the user */
	cout << endl << "Your first matrix: " << endl << endl;
	m1.printMatrix();
	...
	/* Printing the second matrix to the user.  */
	cout << endl << "Your second matrix: " << endl << endl;
	m2.printMatrix();
	...
	/* Printing the resulting matrix to the user. */
	cout << endl << "The sum of the two matricies you entered is: " << endl << endl; 
	m3.printMatrix();
 }
